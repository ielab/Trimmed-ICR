...
try:
    ...
    if os.path.exists(icr_path):
        ...
        ICR_AVAILABLE = True
        print("[Listwise] ICR components loaded successfully for attention scoring.")
        ...
        try:
            ...
            if os.path.exists(icr_base_path):
                ...
                print("[Listwise] ICR InContextReranker class loaded successfully.")
            else:
                raise ImportError(f"ICR base path not found: {icr_base_path}")
        except Exception as e2:
            print(f"[Listwise] Warning: ICR InContextReranker class not available. ICRListwiseRanker will not work. Error: {e2}")
except ImportError as e:
    print(f"[Listwise] Warning: ICR components not available. Attention scoring will not work. Error: {e}")
        ...
        if self.scoring == 'generation':
            ...
            with torch.no_grad():
                if self.config.model_type == 't5':
                    ...
                    numeric_token_ids = self.numeric_token_ids[:len(docs)].to(self.device)
                    scores = distributions[numeric_token_ids]
                    ranked = sorted(zip([f"[{str(i+1)}]" for i in range(len(docs))], scores), key=lambda x: x[1], reverse=True)
                    output = '>'.join([r[0] for r in ranked])
                else:
...
class ICRListwiseRanker(ListwiseLlmRanker):
    def __init__(self,
                 icr_model,
                 window_size=100,
                 step_size=1,
        ...
        print(f"[ICRListwiseRanker] Initialized with window_size={window_size}, step_size={step_size}, "
              f"num_repeat={num_repeat}, scoring_strategy={scoring_strategy}")
    
    def compare(self, query: str, docs: List):
        self.total_compare += 1
        
        if len(docs) == 0:
            print("[ICRListwiseRanker] Warning: Empty document list in compare, returning '[1]'.")
            return "[1]"
        ...
        for idx, doc in enumerate(docs_to_process):
            ...
            if doc.text is None:
                print(f"[ICRListwiseRanker] Warning: Document {original_idx} has None text, skipping.")
                continue
            doc_text = str(doc.text).strip()
            if len(doc_text) == 0:
                print(f"[ICRListwiseRanker] Warning: Document {original_idx} has empty text, skipping.")
                continue
            doc_texts.append(doc_text)
            valid_doc_indices.append(original_idx)
        
        if len(doc_texts) == 0:
            print("[ICRListwiseRanker] Warning: All documents filtered, returning default order '[1] > [2] > ...'.")
            return '>'.join([f"[{i+1}]" for i in range(len(docs))])
        
        if len(doc_texts) != len(docs):
            print(f"[ICRListwiseRanker] Warning: {len(docs) - len(doc_texts)} documents were filtered. "
                  f"Original: {len(docs)}, After filtering: {len(doc_texts)}")
        
        try:
            query_str = str(query).strip() if query is not None else ""
            if len(query_str) == 0:
                print("[ICRListwiseRanker] Warning: Empty query, returning default order.")
                return '>'.join([f"[{i+1}]" for i in range(len(docs))])
            ...
            if hasattr(self.icr_model, 'prompt_separator'):
                ...
                if not isinstance(prompt_sep, str):
                    print(f"[ICRListwiseRanker] Warning: prompt_separator is not a string! Type: {type(prompt_sep)}, Value: {repr(prompt_sep)}")
            ...
            if not sorted_indices or len(sorted_indices) == 0:
                print("[ICRListwiseRanker] Warning: ICR returned empty sorted_indices, returning default order.")
                return '>'.join([f"[{i+1}]" for i in range(len(docs))])
            
            ranked_labels = []
            for idx_in_doc_texts in sorted_indices:
                if idx_in_doc_texts < len(valid_doc_indices):
                    original_idx = valid_doc_indices[idx_in_doc_texts]
                else:
                    print(f"[ICRListwiseRanker] Warning: Index {idx_in_doc_texts} out of range for valid_doc_indices (len={len(valid_doc_indices)}), skipping.")
                    continue
                
                if original_idx < 0 or original_idx >= len(docs):
                    print(f"[ICRListwiseRanker] Warning: Invalid original_idx {original_idx} for {len(docs)} docs, skipping.")
                    continue
                
                ranked_labels.append(f"[{original_idx + 1}]")
            
            if len(ranked_labels) < len(docs):
                ranked_indices_set = set()
                for idx_in_doc_texts in sorted_indices:
                    if idx_in_doc_texts < len(valid_doc_indices):
                        ranked_indices_set.add(valid_doc_indices[idx_in_doc_texts])
                
                for i in range(len(docs)):
                    if i not in ranked_indices_set:
                        ranked_labels.append(f"[{i + 1}]")
            ...
            print(f"[ICRListwiseRanker] Error in compare: {e}")
    ...
    def truncate(self, text, length):
        if hasattr(self.icr_model, 'tokenizer') and self.icr_model.tokenizer is not None:
            tokens = self.icr_model.tokenizer.tokenize(text)
            truncated_tokens = tokens[:length]
            return self.icr_model.tokenizer.convert_tokens_to_string(truncated_tokens)
        else:
