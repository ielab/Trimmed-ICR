...
MODE=${3:-generation}
METHOD=${4:-heapsort}
MAX_LAYER=${5:-}  # default: None
AGGREGATION_START_LAYER=${6:-}  # default: None
SCORING_STRATEGY=${7:-masked_NA_calibration}  #default: masked_NA_calibration

echo "Running BEIR Setwise experiment on dataset: ${DATASET}"
...
SETWISE_RUN="run.setwise.${METHOD}.${DATASET}.${MODEL_TAG}.${MODE}${LAYER_SUFFIX}.txt"
echo "Running setwise reranking with method=${METHOD} and scoring=${MODE}..."

python run.py \
  run --model_name_or_path ${MODEL_NAME} \
      --tokenizer_name_or_path ${MODEL_NAME} \
      --run_path ${BM25_RUN} \
      --save_path ${SETWISE_RUN} \
...
abbrev_method() {
  case "$1" in
    heapsort) echo "heap" ;;
    bubblesort) echo "bubble" ;;
    *) echo "$1" ;;
  esac
}

MODE_ABBREV=$(abbrev_mode "${MODE}")
METHOD_ABBREV=$(abbrev_method "${METHOD}")

STATS_FILENAME_PARTS=("${DATASET}" "${MODEL_TAG}" "${MODE_ABBREV}" "${METHOD_ABBREV}")
if [ "${MODE}" = "attention" ]; then
  RETRIEVAL_TYPE="IE"
  if [ "${DATASET}" = "trec-covid" ] || [ "${DATASET}" = "fiqa" ] || [ "${DATASET}" = "webis-touche2020" ] || [ "${DATASET}" = "dbpedia-entity" ] || [ "${DATASET}" = "nq" ]; then
    RETRIEVAL_TYPE="QA"
